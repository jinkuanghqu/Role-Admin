...
<body class="iframe-body" data-sid="<?php echo ($adminRow["id"]); ?>" data-settime="<?php echo (C("IFRAME_BODY_SETTIME")); ?>" data-actionurl="/Admin/User/userGrade">
            ...
            <div class="panel-collapse collapse in">
                <div class="panel-body">
                    <div class="row panel-scroll">
                        <div class="col-sm-12">
                            <a href="<?php echo U('/Admin/User/userGradeAddModify');?>" class="btn btn-success ajax-add-iframe">添加</a>
                        </div>
                        <form action="<?php echo U('/Admin/User/gradeDestroy');?>" class="form-horizontal checkbox-del">
                            <div class="col-sm-12">
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="center">等级logo</th>
                                            <th class="center">所需评分</th>
                                            <th class="center">所需金额</th>
                                            <th class="center">操作</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php if(is_array($userGradeRows)): $i = 0; $__LIST__ = $userGradeRows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$userGradeRow): $mod = ($i % 2 );++$i;?><tr class="info">
                                            <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($userGradeRow["id"]); ?>"></input></td>
                                            <td><?php echo ($userGradeRow["grade_name"]); ?></td>
                                            <td><img src="<?php echo (P($userGradeRow["grade_logo"])); ?>" style="max-height: 50px; max-width: 50px;" /></td>
                                            <td>
                                                <?php echo ($userGradeRow["compound_points"]); ?>
                                            </td>
                                            <td><?php echo ($userGradeRow["min_total_order_amount"]); ?></td>
                                            <td>
                                                <a href="<?php echo U('User/userGradeAddModify',array('id'=>$userGradeRow['id']));?>" class="btn btn-sm btn-info">修改</a>
                                                <a href="javascript:void(0);" data-ajax="<?php echo U('User/gradeDestroy',array('id'=>$userGradeRow['id']));?>" class="btn btn-sm btn-danger ajax-destroy">删除</a>
                                            </td>
