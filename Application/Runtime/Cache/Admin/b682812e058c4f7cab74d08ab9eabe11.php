...
<body class="iframe-body" data-sid="<?php echo ($adminRow["id"]); ?>" data-settime="<?php echo (C("IFRAME_BODY_SETTIME")); ?>" data-actionurl="/Admin/Admin/index">
            ...
            <div class="panel-collapse collapse in">
                <div class="panel-body">
                    <div class="row panel-scroll">                        
                        <div class="col-sm-12">
                            <a href="<?php echo U('/Admin/Admin/add');?>" class="btn btn-success ajax-add-iframe">添加</a>
                        </div>
                        <form action="<?php echo U('/Admin/Admin/destroy');?>" class="form-horizontal checkbox-del">
                            <div class="col-sm-12">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        <?php if(is_array($adminRows)): $i = 0; $__LIST__ = $adminRows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$adminRow): $mod = ($i % 2 );++$i;?><tr class="info">                                       
                                            <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($adminRow["id"]); ?>"></input></td>
                                            <td><?php echo ($adminRow["name"]); ?></td>
                                            <td><?php echo ($adminRow["email"]); ?></td>
                                            <td><?php echo ($adminRow["roleRow"]["name"]); ?></td>
                                            <td>
                                                <?php if($adminRow['status'] == 0): ?><span class="label label-danger">否</span>
                                                <?php else: ?>
                                                <span class="label label-danger">是</span><?php endif; ?>
                                            </td>
                                            <td><?php echo (date("y-m-d",$adminRow["update_time"])); ?></td>
                                            <td>
                                                <a href="<?php echo U('/Admin/Admin/edit',array('id'=>$adminRow['id']));?>" class="btn btn-sm btn-info">修改</a>                                
                                                <a href="javascript:void(0);" data-ajax="<?php echo U('/Admin/Admin/destroy',array('id'=>$adminRow['id']));?>" class="btn btn-sm btn-danger ajax-destroy">删除</a>
                                            </td>
