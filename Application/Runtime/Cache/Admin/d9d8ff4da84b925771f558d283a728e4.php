...
<body class="iframe-body" data-sid="<?php echo ($adminRow["id"]); ?>" data-settime="<?php echo (C("IFRAME_BODY_SETTIME")); ?>" data-actionurl="/Admin/ArticleTags/edit?id=2">
    <div class="console-container ng-scope">
        ...
        <div class="row">
    <div class="col-sm-8">
        <form action="/admin/ArticleTags/edit" method="POST" id="formData" data-ajax='true' class="form-horizontal autoBootstrapValidator">
        <input type="hidden" name="id" value="<?php echo ($oneData['id']); ?>">
            <div class="form-group">
                <label for="name" class="col-sm-3 control-label">标签名称：</label>
                <div class="col-sm-5">
                    <input type="text" class="form-control" name="name" check-type="required" value="<?php echo ($oneData['name']); ?>" placeholder="标签名称">
                </div>
            </div>
            <div class="form-group">
                <label for="sort" class="col-sm-3 control-label">排序：</label>
                <div class="col-sm-5">
                    <input type="number" class="form-control" name="sort" value="<?php echo ($oneData['sort']); ?>" placeholder="数字,越小越靠前">
                </div>
            </div>
             <div class="form-group">
                <label for="isdisplay" class="col-sm-3 control-label">状态</label>            
                <div class="col-sm-5">
                <select id="isdisplay" name="isdisplay" class="form-control">
                    <option value="1" <?php if($oneData['isdisplay'] == 1): ?>selected<?php endif; ?>>显示</option>
                    <option value="0" <?php if($oneData['isdisplay'] == 0): ?>selected<?php endif; ?>>隐藏</option>
                </select>
                </div>
            </div> 
            <div class="form-group">
